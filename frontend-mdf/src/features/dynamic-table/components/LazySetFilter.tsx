...
//         setSelected(data.filter((v) => existing.values.includes(v)));
//         return;
//       }
...
//         setSelected(
//           data.filter((v) => String(v).toLowerCase().includes(keyword)),
//         );
...
//     load();
//   }, [field, props.context.entityId, JSON.stringify(apiRef.getFilterModel())]);
...
//     const finalValues = selected.filter((v) => scope.includes(v));
...
//   // =========================
//   // CLEAR FILTER (NO DOUBLE REQUEST FIX)
//   // =========================
//   const handleClear = () => {
//     if (!field) return;
...
//     // 🔥 track cleared column (for BE default filter logic)
//     if (!clearedFiltersRef.current.includes(field)) {
//       clearedFiltersRef.current.push(field);
//     }

//     // UI reset only (NO REFRESH REQUEST HERE!)
//     setSelected(values);
//     setSearch("");
//     setPasteText("");
...
//   // =========================
//   return (
//     <div className="w-[280px] rounded-md border bg-white shadow-lg overflow-hidden">
//       {/* HEADER */}
...
//             className="w-full border rounded p-2 text-xs h-[160px]"
//             placeholder={`value1\nvalue2\nvalue3`}
//           />
